...
test("iPhone header removes the redundant wordmark and keeps native glass controls", async () => {
  const dashboard = await readFile(
    new URL("../ios/CodexDeckMobile/Views/DashboardView.swift", import.meta.url), "utf8");
  assert.doesNotMatch(dashboard, /Text\("CODEX"\)/);
  assert.doesNotMatch(dashboard, /Text\("MICRO"\)/);
  assert.match(dashboard, /private struct DashboardTopBar/);
  assert.match(dashboard, /ConnectionCapsule\(\)/);
  assert.match(dashboard, /\.glassEffect\(\.regular\.interactive\(\), in: \.circle\)/);
});

test("iPhone landscape uses a square bezel-free control surface and edge drawer", async () => {
  const dashboard = await readFile(
    new URL("../ios/CodexDeckMobile/Views/DashboardView.swift", import.meta.url), "utf8");
  assert.match(dashboard, /let isLandscape = proxy\.size\.width > proxy\.size\.height/);
  assert.match(dashboard, /let side = min\(size\.height, size\.width\)/);
  assert.match(dashboard, /microDevice\(presentation: \.bezelFree\)/);
  assert.match(dashboard, /\.frame\(width: side, height: side\)/);
  assert.match(dashboard, /isExpanded: \$landscapeDrawerExpanded/);
  assert.match(dashboard, /min\(330, size\.width \* 0\.42\)/);
  const device = await readFile(
    new URL("../ios/CodexDeckMobile/Views/MicroDeviceView.swift", import.meta.url), "utf8");
  assert.match(device, /enum CodexMicroPresentation:[\s\S]*case framed[\s\S]*case bezelFree/);
  assert.match(device, /if presentation == \.framed \{[\s\S]*DeviceEnclosure/);
  assert.match(device, /\.aspectRatio\(1, contentMode: \.fit\)/);
  assert.match(device, /presentation == \.bezelFree \? 30 : 43/);
});
...
test("iPhone commands use stable thread transactions without fake receipt notifications", async () => {
  ...
  assert.doesNotMatch(dashboard, /CommandReceiptHUD/);
  assert.match(dashboard, /if let toast = store\.toast/);
