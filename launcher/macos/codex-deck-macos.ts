...
async function configureMobileLocal(
  portValue: string | undefined,
  rotate: boolean,
  openPairingCode = true
): Promise<void> {
  ...
  if (openPairingCode) run("/usr/bin/open", [result.qrPath], { allowFailure: true });
}
...
async function main(): Promise<number> {
  ...
  if (command === "mobile-local-config") {
    await configureMobileLocal(
      process.argv.find((value, index) => index > 2 && /^\d+$/.test(value)),
